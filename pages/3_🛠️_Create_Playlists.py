"""
3_🛠️_Create_Playlists.py – Classifica AI e Crea Playlist su Spotify
"""
...
if not tracks:
    st.warning("⚠️ Nessuna traccia da processare. Torna alla Home.")
    st.stop()
...
def create_playlists_action(target_buckets, user_choices):
    ...
    existing_playlists_list = [] # Cache per get_or_create_playlist
    
    if "user_playlists" in st.session_state:
        # user_playlists è {name: id}, invertiamolo per lookup
        playlists_map_id_to_name = {v: k for k, v in st.session_state["user_playlists"].items()}
        # Ricostruiamo una lista dummy per la cache di get_or_create_playlist
        # (A get_or_create_playlist serve solo 'name' e 'id' quindi basta questo)
        existing_playlists_list = [{"name": k, "id": v} for k, v in st.session_state["user_playlists"].items()]
    ...
    try:
        ...
        for idx, (cat_name, track_list) in enumerate(target_buckets.items(), 1):
             ...
             if choice_id == "NEW":
                 ...
                 playlist_id = get_or_create_playlist(
                    sp, user_id, cat_name,
                    description=f"Auto-generated by AlgoRhythm 🎵 ({len(track_list)} tracks)",
                    existing_playlists_cache=existing_playlists_list
                 )
                 ...
                 added = add_missing_tracks_to_playlist(sp, playlist_id, uris)
                 action_type = f"Updated (+{added} nuovi)"

             created_now.append({"Playlist": target_name_display, "Action": action_type, "Tracks": len(uris)})
             updated_count += 1

        st.success(f"✅ Operazione completata! {updated_count} playlist processate.")
        st.balloons()
        ...
        # Pulisci cache delle playlist per forzare aggiornamento al prossimo uso
        if "user_playlists" in st.session_state:
             del st.session_state["user_playlists"]
             
    except Exception as e:
        st.error(f"Errore durante l'operazione: {e}")
        # Loggare l'errore completo in console è utile per debug
        print(f"Error in create_playlists_action: {e}")


# ── UI ──

st.title("🛠️ Laboratorio Playlist")

# Carica cache classificazione da disco se non già in sessione
if "classifications" not in st.session_state or not st.session_state["classifications"]:
    cache_file = "user_data/classification_cache.json"
    if os.path.exists(cache_file):
        try:
            with open(cache_file, "r", encoding="utf-8") as f:
                cached_classifications = json.load(f)
            if cached_classifications:
                st.session_state["classifications"] = cached_classifications
                st.session_state["genre_buckets"] = build_genre_buckets(tracks, cached_classifications)
        except Exception as e:
...
with tab1:
    ...
    if st.button("▶️ Avvia Analisi AI"):
        ...
        labels = [
            t.get("label", f"{t.get('artist','')} - {t.get('name','')}")
            for t in unclassified
        ]
        st.info(f"Classifico **{len(unclassified)}** brani nuovi · già classificati: {len(tracks) - len(unclassified)}")

        container = st.container()
        progress = st.progress(0)

        gen = classify_all_tracks(labels)
        ...
        try:
            with open("user_data/classification_cache.json", "w", encoding="utf-8") as f:
                json.dump(current_class, f, ensure_ascii=False)
        except Exception as e:
    ...
    st.markdown("---")
    st.markdown("### 🔁 Riclassifica i brani 'To Review'")

    genre_buckets_current = st.session_state.get("genre_buckets", {})
    to_review = genre_buckets_current.get("⚠️ To Review", [])

    if not to_review:
        st.info("Nessun brano in 'To Review'. Esegui prima l'analisi AI.")
    else:
        st.write(f"**{len(to_review)}** brani non classificati correttamente:")
        with st.expander("Vedi brani in To Review", expanded=False):
            for t in to_review:
                st.caption(f"• {t.get('artist','')} - {t.get('name','')}")

        if st.button("🔁 Riclassifica 'To Review' con AI"):
            current_class = st.session_state.get("classifications", {})
            labels = [
                t.get("label", f"{t.get('artist','')} - {t.get('name','')}")
                for t in to_review
            ...
            # Rimuovi dalla cache per forzare riclassificazione
            for lbl in labels:
                current_class.pop(lbl, None)

            st.info(f"Riclassifico **{len(labels)}** brani...")
            container = st.container()
            progress = st.progress(0)
            gen = classify_all_tracks(labels)
            ...
            st.success("Riclassificazione completata!")
...
with tab2:
    ...
    def render_creation_section(title, buckets, key_prefix):
        ...
        for cat_name, track_list in buckets.items():
            ...
            with st.expander(f"📂 **{cat_name}** ({len(track_list)} brani)", expanded=False):
                # Anteprima brani
    ...
    with col_a:
        user_choices_decades = render_creation_section("📅 Playlist Decadi", year_buckets, "decades")
        if user_choices_decades and st.button("🚀 Avvia Creazione (Decadi)", key="btn_start_decades"):
             create_playlists_action(year_buckets, user_choices_decades)

    with col_b:
        user_choices_genres = render_creation_section("🎭 Playlist Generi AI", genre_buckets, "genres")
        # Mostra il pulsante solo se ci sono buckets
        if user_choices_genres is not None:
             if st.button("🚀 Avvia Creazione (Generi)", key="btn_start_genres"):
