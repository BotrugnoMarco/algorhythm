...
def create_playlists_action(target_buckets, user_choices):
    ...
    try:
        ...
        for idx, (cat_name, track_list) in enumerate(target_buckets.items(), 1):
             ...
             # CHECK SE GIA' FATTO DI RECENTE (opzionale, ma utile per resume manuale)
             # Per ora ci affidiamo al fatto che se l'utente rilancia, magari vuole aggiornare di nuovo.
             # Ma se si blocca a metà, l'utente rilancia e noi dovremmo saltare quelli già fatti in QUESTA "sessione logica"
             # Implementiamo un salvataggio incrementale dello stato
             ...
             if choice_id == "NEW":
                 ...
                 try:
                    ...
                    st.error(f"Errore creazione playlist '{cat_name}': {e}")
                    continue # Prova con la prossima
             else:
             ...
             try:
                ...
                st.error(f"Errore aggiunta tracce a '{target_name_display}': {e}")
                continue

             # LOGICA RESUME: Salviamo IMMEDIATAMENTE nello storico
             single_entry = [{"Playlist": target_name_display, "Action": action_type, "Tracks": len(uris)}]
             _save_created_history(single_entry)
             
             # Aggiorniamo anche la session state per feedback immediato
             if "created_info" not in st.session_state: st.session_state["created_info"] = []
             st.session_state["created_info"] = st.session_state["created_info"] + single_entry
