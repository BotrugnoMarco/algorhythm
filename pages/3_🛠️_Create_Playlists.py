...
# ── LOGIC FUNCTIONS (COPIED FROM APP.PY) ──
...
def create_playlists_action(mode="all"):
    ...
    st.subheader(f"🚀 Creazione in corso ({mode})...")
    ...
    try:
        for idx, (name, bucket) in enumerate(all_buckets.items(), 1):
            ...
            pid = get_or_create_playlist(
                sp, user_id, name, 
                description=f"Auto-generated by AlgoRhythm 🎵 ({len(bucket)} tracks)"
            )
        ...
        if e.http_status == 403:
            st.error("🚨 ERRORE PERMESSI (403).")
            st.error("Il tuo token non ha i permessi per creare playlist o l'utente non è autorizzato.")
    ...
    st.success(f"✅ Create {len(created_now)} playlist!")
    st.balloons()
...
with tab2:
    ...
    with col_a:
        st.info("📅 **Playlist Decadi** (Anni '90, '00...)")
    ...
    with col_b:
        st.info("🎭 **Playlist Generi AI** (Mood, Vibe...)")
        if genre_buckets:
            ...
            st.warning("⚠️ Esegui prima l'analisi AI nel Tab 1!")
